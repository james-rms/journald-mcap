...
int sd_journal_seek_realtime_usec(sd_journal *j, uint64_t usec) {
  j->entry_cursor_valid = false;
  for (j->entry_cursor = 0; j->entry_cursor < (int)(j->entry_timestamps.size());
       j->entry_cursor++) {
...
int sd_journal_next(sd_journal *j) {
  ...
  if (j->entry_cursor < 0 ||
      j->entry_cursor >= (int)(j->entry_timestamps.size())) {
    j->entry_cursor_valid = false;
    return 0;
  }
  return 1;
...
int sd_id128_get_boot(sd_id128_t *out) {
  *out = SD_ID128_MAKE(00, 01, 02, 03, 04, 05, 06, 07, 08, 09, 0a, 0b, 0c, 0d,
                       0e, 0f);
  return 0;
}
